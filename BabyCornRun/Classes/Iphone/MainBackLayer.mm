...
@implementation MainBackLayer

+(id) scene
{
    CCScene *scene = [CCScene node];
    
    MainBackLayer *layer = [MainBackLayer node];
...
-(id) init
{
    ...
    if( (self=[super init] )) {
       ...
       //world->SetDebugDraw(m_debugDraw);
        ...
        if (isFirstMain){
            isFirstMain = false;
            groundTex = [[CCTextureCache sharedTextureCache] textureForKey:@"ground_004.png"];
        }
        else {
            groundTex = [[CCTextureCache sharedTextureCache] addImage:@"ground_004.png"];
        }
