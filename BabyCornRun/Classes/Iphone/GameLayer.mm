...
-(void)spawnObstacleWithWorld:(b2Vec2)groundPos
{
    ...
    obstacleSprite = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:@"scarecrow.png"]];
    ...
    b2Vec2 verts[20];    
    int32 num = 6;
    verts[0].Set(-107.0f / ptm_ratio, -30.5f / ptm_ratio);
    verts[1].Set(108.0f / ptm_ratio, -30.5f / ptm_ratio);
    verts[2].Set(108.0f / ptm_ratio, 11.5f / ptm_ratio);
    verts[3].Set(88.0f / ptm_ratio, 32.5f / ptm_ratio);
    verts[4].Set(-84.0f / ptm_ratio, 31.5f / ptm_ratio);
    verts[5].Set(-107.0f / ptm_ratio, 14.5f / ptm_ratio);
...
-(void)spawnCorn
{
    ...
    CCSprite *cornSprite = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:[NSString stringWithFormat: @"cornlife.png"]]];
        
    num = 8;
    verts[0].Set(-11.5f / ptm_ratio, 9.4f / ptm_ratio);
    verts[1].Set(-9.7f / ptm_ratio, -6.5f / ptm_ratio);
    verts[2].Set(-0.9f / ptm_ratio, -13.3f / ptm_ratio);
    verts[3].Set(8.0f / ptm_ratio, -5.8f / ptm_ratio);
    verts[4].Set(8.7f / ptm_ratio, 7.6f / ptm_ratio);
    verts[5].Set(4.1f / ptm_ratio, 17.9f / ptm_ratio);
    verts[6].Set(0.5f / ptm_ratio, 20.3f / ptm_ratio);
    verts[7].Set(-10.8f / ptm_ratio, 13.3f / ptm_ratio);
...
{  
    ...
    switch (randomTerrain) {
        ...
        { 
            ...
            nextloc=b2Vec2(groundBody->GetPosition().x+11.125+3,0);
            break;
        }
        case 1:
        { 
        ...
        case 2:
        {
            ...
            nextloc=b2Vec2(groundBody->GetPosition().x+4.08+3,0);
            break;
        }
        case 3:
        {  
        ...
        { 
            ...
            nextloc=b2Vec2(groundBody->GetPosition().x+3.92+3,0);
            break;
        }
        case 6:
        {
            groundBox.Set(b2Vec2(-11.06+1.0,-0.1), b2Vec2(+11.06,-0.1));
            nextloc=b2Vec2(groundBody->GetPosition().x+11.06+3,0);
            break;
        }
        case 7:
        {  
            groundBox.Set(b2Vec2(-8.92+1.0,-0.1), b2Vec2(+8.92,-0.1));
            nextloc=b2Vec2(groundBody->GetPosition().x+8.92+3,0);
            break;
        }
        case 8:
        {  
            groundBox.Set(b2Vec2(-3.45+1.0,-0.1), b2Vec2(+3.45,-0.1));
            nextloc=b2Vec2(groundBody->GetPosition().x+3.45+3,0);
            break;
        }
        default:
            break;
...
-(void) spawnEnemies{
    ...
    b2Vec2 verts[20];
    num = 5;
    verts[0].Set(-23.5f / ptm_ratio, -3.0f / ptm_ratio);
    verts[1].Set(-0.5f / ptm_ratio, -16.0f / ptm_ratio);
    verts[2].Set(22.5f / ptm_ratio, -3.0f / ptm_ratio);
    verts[3].Set(17.5f / ptm_ratio, 18.0f / ptm_ratio);
    verts[4].Set(-8.5f / ptm_ratio, 19.0f / ptm_ratio);
...
- (void) loadData {
    ...
    if (!isLoadedData) {
        ...
        for (int cnt = 0; cnt < MAX_GROUNDS; cnt++) {
            CCTexture2D *groundTex = [[CCTextureCache sharedTextureCache] addImage:[NSString stringWithFormat:@"ground_00%d.png",cnt]];
...
- (void)ccTouchesBegan:(NSSet *)touch withEvent:(UIEvent *)event 
{
    ...
    for (UITouch * touch in allTouches)
    {
        ...
        {   
            ...
            CGRect pause = CGRectMake(400, 250, 83, 79);
