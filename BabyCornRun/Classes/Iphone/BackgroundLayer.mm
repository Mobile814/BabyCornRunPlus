...
-(id)init
{
        ...
        
    {
        ...
        cloud1 = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:@"cloud2.png"]];
        cloud1.tag = kTagCloud1;
        cloud1.anchorPoint = ccp(0.0, 1.0);
        cloud1.position = ccp(140, screenSize.height-20);
        [self addChild:cloud1 z:11];
        
        cloud2 = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:@"cloud1.png"]];
        cloud2.tag = kTagCloud2;
        cloud2.anchorPoint = ccp(0.0, 1.0);
        cloud2.position = ccp(200, screenSize.height-40);
        [self addChild:cloud2 z:11];
        
        cloud3 = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:@"cloud2.png"]];
        cloud3.tag = kTagCloud3;
        cloud3.anchorPoint = ccp(0.0, 1.0);
        cloud3.position = ccp(270, screenSize.height-30);
        [self addChild:cloud3 z:11];
        
        cloud4 = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:@"cloud1.png"]];
        cloud4.tag = kTagCloud4;
        cloud4.anchorPoint = ccp(0.0, 1.0);
        cloud4.position = ccp(180, screenSize.height-80);
        [self addChild:cloud4 z:11];
        
        cloud5 = [CCSprite spriteWithTexture:[[CCTextureCache sharedTextureCache] addImage:@"cloud2.png"]];
        cloud5.tag = kTagCloud5;
        cloud5.anchorPoint = ccp(0.0, 1.0);
        cloud5.position = ccp(210, screenSize.height-60);
...
-(void)updateCloudPosition :(CCSprite*)cloud :(CGPoint) lastPosition :(CGPoint) currentPosition
{
    ...
    if (cloud.position.x+cloud.contentSize.width+120 < currentPosition.x) 
    {   
        cloud.position = ccpAdd(cloud.position, ccp(480+cloud.contentSize.width, 0));
    }
    else{
